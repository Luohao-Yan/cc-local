...
| QueryEngine | ✅ 基础 | Mock 实现，待接入实际 AI API |
| HTTP 服务端 | ✅ 完成 | Bun HTTP + SSE 流式响应 |
| WebSocket | ✅ 完成 | 双向通信支持 |
| CLI 客户端 | ✅ 基础 | 连接服务端，简化版 REPL |
| VS Code 扩展 | 🚧 原型 | WebSocket 客户端实现，需调试 |

## 下一步工作

1. **接入实际 AI API** - 将 QueryEngine 连接到 Claude/Doubao API
2. **工具系统集成** - 迁移现有工具到 core 包
3. **会话持久化** - 添加 SQLite 存储
4. **认证完善** - 实现更安全的认证机制
5. **VS Code 扩展调试** - 解决构建问题，完整集成
6. **测试覆盖** - 添加单元测试和集成测试
