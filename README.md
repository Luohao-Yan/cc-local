...
- [REST API 与 MCP](#rest-api-与-mcp)
  - [启动本地服务端](#启动本地服务端)
  - [使用 Packages-Native 入口](#使用-packages-native-入口)
  - [服务器端点](#服务器端点)
  - [并发优化特性](#并发优化特性)
- [终端宠物伴侣](#终端宠物伴侣buddy)
...
如果你已单独启动了服务器，或需要连接远程 CCLocal server：
...
# 使用服务器（终端 2）
cclocal --server http://127.0.0.1:5678 --token <token> sessions list
cclocal --server http://127.0.0.1:5678 --token <token> mcp list
cclocal --server http://127.0.0.1:5678 --token <token> models list

# 或使用 packages-native 入口
...
- `packages/server`: 本地 HTTP + SSE 服务端，支持并发优化
...
✅ Server ready at http://127.0.0.1:5678
   ...
   API token: <auto-generated-token>
```
...
### 使用 Packages-Native 入口

Packages-Native 架构提供独立的 REST 客户端入口，包含完整的交互式 REPL：
...
# 启动 REPL 并指定工作目录
bun run packages/cli/src/entrypoints/native.ts --cwd /path/to/project

# 启动 REPL 并指定模型
bun run packages/cli/src/entrypoints/native.ts --model claude-sonnet-4
...
|------|------|------|
| `GET /health` | GET | 健康检查 |
| `GET /api/v1/sessions` | GET | 列出所有会话 |
| `POST /api/v1/sessions` | POST | 创建新会话 |
| `GET /api/v1/sessions/:id` | GET | 获取会话详情 |
| `DELETE /api/v1/sessions/:id` | DELETE | 删除会话 |
| `POST /api/v1/sessions/:id/messages` | POST | 发送消息（SSE 流） |
| `GET /api/v1/models` | GET | 列出可用模型 |
| `GET /api/v1/mcp/servers` | GET | 列出 MCP 服务器 |
| `POST /api/v1/mcp/servers` | POST | 添加 MCP 服务器 |
| `POST /api/v1/mcp/servers/:name/connect` | POST | 连接 MCP 服务器 |
| `POST /api/v1/mcp/servers/:name/disconnect` | POST | 断开 MCP 服务器 |
...
# 创建会话
curl -X POST http://localhost:5678/api/v1/sessions \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"name":"My Session","model":"claude-sonnet-4"}'

# 发送消息（SSE 流）
curl -N -X POST http://localhost:5678/api/v1/sessions/<session-id>/messages \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
...
- 支持 100+ 并发任务（请求队列、工具并发限制、WebSocket 连接管理）

### 并发优化特性

Packages-Native 架构已针对高并发场景进行优化：

| 特性 | 说明 |
|------|------|
| 请求队列 | 同一会话的请求串行处理，避免竞态条件 |
| 智能工具并发 | 只读工具（file_read, grep 等）最多 10 个并行；写入工具串行执行 |
| WebSocket 管理 | 最大 500 连接，空闲超时自动清理 |
| 数据库 WAL | 默认启用 WAL 模式，提升并发性能 |
| 速率限制 | 令牌桶算法，每分钟/每秒请求限制 |

**工具并发控制**：

```bash
# 默认最大并发数：10（与原版 Claude Code 一致）
# 可通过环境变量调整
...
```bash
export CCLOCAL_API_KEY=your-fixed-token
bun run start:server
```

### Packages-Native 入口有什么用

Packages-Native 入口 (`packages/cli/src/entrypoints/native.ts`) 提供独立的 REST 客户端：

- 支持连接本地或远程服务器
- 提供完整的 REST 命令支持（sessions、mcp、models、doctor）
- 适合脚本集成或远程管理
