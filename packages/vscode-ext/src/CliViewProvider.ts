...
import { CclocalProcess } from './CclocalProcess.js'
import type {
  CclocalStatus,
...
export class CliViewProvider implements vscode.WebviewViewProvider {
  ...
  private cclocalProcess: CclocalProcess
  /** 当前正在构建的 AI 消息 ID */
  private currentMessageId = ''
  /** 当前状态 */
  private status: CclocalStatus = 'idle'
   ...
   */
  public handleCommand(command: 'newSession' | 'clearChat' | 'stopGeneration'): void {
    switch (command) {
      case 'newSession':
        this.newSession()
        break
  ...
  private handleSendMessage(text: string): void {
    ...
    const cclocalPath: string = config.get('cclocalPath') || 'cclocal'
  ...
  /** 更新状态并通知 Webview */
  private setStatus(status: CclocalStatus): void {
    this.status = status
