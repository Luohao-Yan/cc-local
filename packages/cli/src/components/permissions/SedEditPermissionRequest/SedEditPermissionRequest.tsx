...
function SedEditPermissionRequestInner(t0) {
  ...
  bb1: {
    if (!fileExists) {
      t3 = t('sedEdit.fileDoesNotExist');
      break bb1;
    }
    t3 = t('sedEdit.patternNotMatched');
  }
  ...
  let t11;
  if ($[18] !== t10) {
    t11 = <Text>{t('sedEdit.doYouWantToMakeEdit')}{" "}<Text bold={true}>{t10}</Text>?</Text>;
    $[18] = t10;
    $[19] = t11;
  } else {
    t11 = $[19];
  ...
  if ($[24] !== filePath || $[25] !== parseInput || $[26] !== props.onDone || $[27] !== props.onReject || $[28] !== props.toolUseConfirm || $[29] !== props.toolUseContext || $[30] !== props.workerBadge || $[31] !== t11 || $[32] !== t12 || $[33] !== t9) {
    t13 = <FilePermissionDialog toolUseConfirm={t5} toolUseContext={t6} onDone={t7} onReject={t8} title={t('sedEdit.title')} subtitle={t9} question={t11} content={t12} path={filePath} completionType="str_replace_single" parseInput={parseInput} workerBadge={props.workerBadge} />;
    $[24] = filePath;
    $[25] = parseInput;
    $[26] = props.onDone;
    $[27] = props.onReject;
