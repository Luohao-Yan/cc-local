...
const REMEMBER_DIRECTORY_OPTIONS: Array<{
  ...
  value: 'yes-session',
  label: t('rules.yesForThisSession')
}, {
  value: 'yes-remember',
  label: t('rules.yesAndRemember')
}, {
  value: 'no',
  label: t('rules.no')
}];
function PermissionDescription() {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t0 = <Text dimColor={true}>{t('rules.willHaveAccess')}</Text>;
    $[0] = t0;
  } else {
    t0 = $[0];
  }
...
function DirectoryInput(t0) {
  ...
  let t1;
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t1 = <Text>{t('rules.enterPath')}</Text>;
    $[0] = t1;
  } else {
    t1 = $[0];
  }
  ...
  if ($[1] !== onChange || $[2] !== onSubmit || $[3] !== value) {
    t2 = <Box borderDimColor={true} borderStyle="round" marginY={1} paddingLeft={1}><TextInput showCursor={true} placeholder={t('rules.directoryPath', {ellipsis: figures.ellipsis})} value={value} onChange={onChange} onSubmit={onSubmit} columns={80} cursorOffset={value.length} onChangeCursorOffset={_temp} /></Box>;
    $[1] = onChange;
    $[2] = onSubmit;
    $[3] = value;
    $[4] = t2;
...
export function AddWorkspaceDirectory(t0) {
  ...
  if ($[27] !== onCancel || $[28] !== t10 || $[29] !== t11) {
    t12 = <Dialog title={t('rules.addDirectoryTitle')} onCancel={onCancel} color="permission" isCancelActive={false} inputGuide={t10}>{t11}</Dialog>;
    $[27] = onCancel;
    $[28] = t10;
    $[29] = t11;
    $[30] = t12;
...
function _temp2(exitState) {
  return exitState.pending ? <Text>{t('rules.pressAgainToExit', {key: exitState.keyName})}</Text> : <Byline><KeyboardShortcutHint shortcut="Tab" action="complete" /><KeyboardShortcutHint shortcut="Enter" action="add" /><ConfigurableShortcutHint action="confirm:no" context="Settings" fallback="Esc" description="cancel" /></Byline>;
}
