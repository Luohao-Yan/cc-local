...
function PromptInput({
  ...
  const handleModelSelect = useCallback((model: string | null, _effort: EffortLevel | undefined) => {
    ...
    // Model activation is handled by onChangeAppState (the single source of
    // truth), which fires synchronously during setAppState — so there is no
    // race window where API calls could use a stale endpoint.
    setAppState(prev => {
