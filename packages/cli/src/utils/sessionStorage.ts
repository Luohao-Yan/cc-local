...
// Use plain string for UUID throughout this file to avoid template literal
// narrowing issues. UUIDs are validated at generation time (crypto.randomUUID).
type UUID = string

/** Cast a runtime string to the crypto UUID branded type. Safe because UUIDs are generated by crypto.randomUUID(). */
function asUUID(s: string | null | undefined): CryptoUUID | null {
  return (s ?? null) as CryptoUUID | null
}
...
class Project {
  ...
  reAppendSessionMetadata(skipTitleRefresh = false): void {
    ...
    const sessionId = getSessionId() as CryptoUUID
    if (!sessionId) return
    ...
    return this.trackWrite(async () => {
      ...
      for (const message of messages) {
        ...
        const transcriptMessage: TranscriptMessage = {
          parentUuid: isCompactBoundary ? null : asUUID(effectiveParentUuid as string),
          logicalParentUuid: isCompactBoundary ? asUUID(parentUuid as string | null | undefined) as TranscriptMessage['logicalParentUuid'] : undefined,
          isSidechain,
  ...
  async insertFileHistorySnapshot(
    ...
    return this.trackWrite(async () => {
      const fileHistoryMessage: FileHistorySnapshotMessage = {
        ...
        messageId: messageId as CryptoUUID,
        snapshot,
        isSnapshotUpdate,
      }
  ...
  async insertContentReplacement(
    ...
    return this.trackWrite(async () => {
      const entry: ContentReplacementEntry = {
        ...
        sessionId: getSessionId() as CryptoUUID,
        agentId,
        replacements,
      }
      await this.appendEntry(entry)
  ...
  async appendEntry(entry: Entry, sessionId: UUID = getSessionId() as CryptoUUID) {
    if (this.shouldSkipPersistence()) {
      return
    }

    const currentSessionId = getSessionId() as CryptoUUID
  ...
  if (segIsLive) {
    ...
    if (head) {
      messages.set(lastSeg.headUuid, {
        ...head,
        parentUuid: lastSeg.anchorUuid as CryptoUUID | null,      })
    }
    ...
    for (const [uuid, msg] of messages) {
      if (msg.parentUuid === lastSeg.anchorUuid && uuid !== lastSeg.headUuid) {
        messages.set(uuid, { ...msg, parentUuid: lastSeg.tailUuid as CryptoUUID | null })
      }
    }
...
function applySnipRemovals(messages: Map<UUID, TranscriptMessage>): void {
  ...
  for (const [uuid, msg] of messages) {
    ...
    messages.set(uuid, { ...msg, parentUuid: resolve(msg.parentUuid) as CryptoUUID | null })
    relinkedCount++
  }
...
function recoverOrphanedParallelToolResults(
  ...
  for (const asst of chainAssistants) {
    ...
    const orphanedSiblings: TranscriptMessage[] = group.filter(s => !seen.has(s.uuid))
...
export async function loadFullLog(log: LogOption): Promise<LogOption> {
  ...
  try {
    ...
    return {
      ...
      leafUuid: (mostRecentLeaf?.uuid ?? log.leafUuid) as CryptoUUID,
...
export async function loadTranscriptFile(
    ...
    for (const entry of entries) {
      ...
      if (isTranscriptMessage(entry)) {
        if (entry.parentUuid && progressBridge.has(entry.parentUuid)) {
          entry.parentUuid = (progressBridge.get(entry.parentUuid) ?? null) as CryptoUUID | null
        }
  ...
  const terminalMessages = allMessages.filter(msg => !parentUuids.has(msg.uuid as CryptoUUID))

  const leafUuids = new Set<CryptoUUID>()
  let hasCycle = false
      ...
      while (current) {
        ...
        if (current.type === 'user' || current.type === 'assistant') {
          if (!hasUserAssistantChild.has(current.uuid)) {
            leafUuids.add(current.uuid as CryptoUUID)
          }
          break
        }
...
export async function getAgentTranscript(agentId: AgentId): Promise<{
  ...
  try {
    ...
    const parentUuids = new Set<CryptoUUID | null>(agentMessages.map(msg => msg.parentUuid))
    const leafMessage = findLatestMessage(
      agentMessages,
      msg => !parentUuids.has(msg.uuid as CryptoUUID | null),
    )
