...
function extractOldTopLevelCommands() {
  const main = read('packages/cli/src/main.tsx')
  return uniqueSorted([
...
function extractOldSlashCommands() {
  const commandFiles = walk('packages/cli/src/commands', (path) => /\.(ts|tsx)$/.test(path))
  const names = []
...
function extractOldTools() {
  const toolFiles = walk('packages/cli/src/tools', (path) => /\.(ts|tsx)$/.test(path))
  const names = []
...
Compatibility note: \`packages/cli\` keeps an explicit \`--legacy\` escape hatch. Top-level commands/options marked as bridged are user-visible through the new CLI but still execute the legacy UI via spawnSync.
